/**
 * services/localTranslations.ts
 *
 * Hardcoded question translations used as a fallback when the DB
 * has no entries. Matches on the English `question_text` and
 * replaces `question_text` and `hint` when a mapping exists.
 */
...
const TRANSLATIONS: Array<{
  en: string;
  hi?: { q: string; h?: string };
  te?: { q: string; h?: string };
}> = [
  ...
  },
  {
    ...
    te: { q: "ln(x) యొక్క అవకలనం ఏమిటి?",           h: "ఏ ప్రమేయం యొక్క సమాకలనం ln(x) ఇస్తుందो ఆలోచించండి." },
  },
  {
  ...
  },
  {
    ...
    te: { q: "cos(x) dx యొక్క సమాకలనం ఏమిటి?",       h: "ఏ త్రिकోణమితి ప్రమేయం యొక్క అవకలనం cos(x)? + C మరవకండి." },
  },
  {
  ...
  },
  {
    ...
    te: { q: "x^3 dx యొక్క సమాకలనం ఏమిటి?",         h: "విలోమ ఘాత నియమం: ఘातాంకం 1 పెంచండి, కొత్త దానితో భాగించండి." },
  },
  ...
  },
  {
    ...
    te: { q: "कర్ణం 10, ఒక భుజం 6 గల లంబकోణ త్రిభుజంలో మరొక భుజం కనుగొనండి",       h: "b = √(c² − a²) వాడండి." },
  },
  {
  ...
  // ── Geometry: Area of Triangles ──────────────────────────────────────────
  {
    ...
    te: { q: "పాదం 10 మరియు ఎత్తు 5 ఉన్న త্রిభుజం యొక్క వైశాల్యం లెక్కించండి",       h: "A = ½ × పాదం × ఎత్తు = ½ × 10 × 5" },
  },
  {
  ...
  },
  {
    ...
    hi: { q: "भुजा 4 वाले समबाहु त्रिभुज का क्षेत्रफल ज्ञात کریں (sqrt के पदों में)", h: "A = (√3/4)s² का उपयोग करें। s = 4 रखें।" },
  ...
  },
  {
    ...
    te: { q: "లంబకోణ త్రిభుజంలో, వ్యతిరేకం = 3, ఆసన్నం = 4. tan(కోణం) ఏమిటి?",       h: "TOA: tan = వ్యతిరేకం/ఆసन्नం. విలువలు పెట్టండి." },
  },
  ...
  // ── Probability: Simple Probability ─────────────────────────────────────
  {
    ...
    te: { q: "6 వైపుల పాచికపై 6 రావడానికి సంభావ్యత (భিন্নం)",                          h: "1 అనుకూల ఫలితం, 6 మొత్తం ఫలితాలు." },
  },
  {
    ...
    te: { q: "6 వైపుల పాచికపై సమ संख्या రావడానికి సంభావ్యత (భిన్నం)",                  h: "సம సంఖ్యలు: 2,4,6 → 6లో 3" },
  },
  {
...
// a simple lookup map for the hi/te fallback data
const MAP = new Map<string, { hi?: { q: string; h?: string }; te?: { q: string; h?: string } }>();
for (const t of TRANSLATIONS) {
  MAP.set(t.en, { hi: t.hi, te: t.te });
}

// number word maps to expand digits into Hindi/Telugu words
...
function convertNumbersToWords(text: string, lang: string): string {
  const map = lang === "hi" ? HINDI_NUMBER_WORDS : TELUGU_NUMBER_WORDS;
...
export function applyHardcodedTranslations(questions: Question[], language: string): Question[] {
  ...
  return questions.map((q) => {
    ...
    const entry = MAP.get(q.question_text);
    if (entry) {
      const tr = (language === "hi" ? entry.hi : entry.te) as { q: string; h?: string } | undefined;
      if (tr) {
        text = tr.q;
        hint = tr.h ?? hint;
      }
    ...
    // Convert standalone numeric tokens into words for the target language
    // (e.g. 5 -> 'पाँच', 15 -> 'एकपाँच'). Preserve attached digits like '2x'.
    if (language === "hi" || language === "te") {
      text = convertNumbersToWords(text, language);
      if (hint) hint = convertNumbersToWords(hint, language);
    }
...
export default applyHardcodedTranslations;
