...
for (const q of SAMPLE_QUESTIONS) {
  // Upsert concept (match on name + subject)
  ...
  if (!concept) {
    const { data: newConcept, error } = await supabase
      ...
      .select()
      .single()
    if (error) { console.error('Concept insert error:', error.message); continue }
    concept = newConcept
    console.log(`  Created concept: [${q.subject}] ${q.name}`)
  }

  const { error: qError } = await supabase
    .from('questions')
    .insert({ concept_id: concept.id, question_text: q.question, correct_answer: q.answer })

  if (qError) {
    console.error(`  Question error (${q.question}):`, qError.message)
    skipped++
  } else {
